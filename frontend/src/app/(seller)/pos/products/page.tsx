...
const SellersProducts = () => {
  ...
  }, [TransactionErrorMessage, transactionError]); // Dependency on transactionError
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div
            className={`grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-4 ${
              cartProducts.cartProducts.length === 0 ? ' md:grid-cols-2 xl:grid-cols-8' : ' md:grid-cols-3 xl:grid-cols-4'
            }`}
          >
        ...
        >
          ...
          {transactionError ? (
            <div className="bg-green-600 text-white p-1 mb-1 text-xl sm:text-2xl">{'transaction error'}</div>
          ) : (
            <>{transactionSuccess ? <div className="bg-green-600 text-white p-1 mb-1 text-xl sm:text-2xl">Success</div> : null}</>
          )}

          <div className="bg-green-600 text-white p-1 mb-1 text-xl sm:text-2xl">Total cost is: {totalcost.total}</div>

          <div className="bg-white p-4 mb-4 rounded-lg shadow-md">Payment Type</div>
