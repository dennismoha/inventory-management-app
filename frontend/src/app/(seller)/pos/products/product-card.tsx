...
const ProductCard = (propsData: ProductsCard) => {
  ...
  console.log('supplier product id is ', supplier_products_id, 'inventory id ', inventoryId);
    ...
    <div className="bg-white p-4 mb-4 rounded-lg shadow-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-300 ease-in-out">
      <div className="flex justify-around items-center mb-4">
        ...
        <div className="flex flex-col justify-center items-center text-center ml-4">
          <p className="text-lg font-light text-gray-700">price: {productPrice ? productPrice : 'no price'}</p>
          <p className="text-lg font-light text-gray-700">VAT: {VAT ? VAT : 'no VAT'}</p>
          <p className="text-lg font-light text-gray-700">discount: {discount ? discount : 'no discount'}</p>
          <p className="text-lg font-light text-gray-700">stock quantity: {stock_quantity ? stock_quantity : 'no stock'}</p>
        </div>
      </div>
...
const ProductList: React.FC<inputData> = (props) => {
  ...
  return (
    <div className="col-span-4 lg:col-span-2 bg-gray-100 p-6 overflow-y-auto">
      <h2 className="text-2xl font-serif mb-4">{props.data.supplierProduct?.supplier?.name}</h2>
