...
const CollapseDefault: React.FC<MyComponentProps> = ({ label, children, isCollapsed, href, Icon }) => {
  ...
  return (
    <>
      <div>
        <div
          className={`cursor-pointer flex items-center ${isCollapsed ? 'justify-center py-4' : 'justify-between px-8 py-4'}
        ...
        >
          <span className={`cursor-pointer flex justify-start`}>
            ...
            <span className={`${isCollapsed ? 'hidden' : 'block'} font-medium text-gray-700 mx-4`}>{label}</span>
          </span>
