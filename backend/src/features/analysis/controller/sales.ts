...
const getSalesPerCustomer = async (): Promise<CustomerSales[]> => {
        ...
        select: {
          ...
          TransactionProduct: {
            select: {
              ...
              productName:true,   
              productTotalCost: true,   
              supplierProduct: { // Include SupplierProduct data
                select: {
                  supplier  : {
                    select :{
                      name:true,                      
  ...
  return salesPerCustomer.map(customer => {
    ...
    const groupedSales = supplierProducts.reduce((acc: { [key: string]: SupplierSales  }, { supplier_products_id,supplierProduct,products, productTotalCost }) => {
